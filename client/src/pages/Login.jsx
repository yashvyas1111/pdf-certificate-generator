...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const res = await axios.post(`${import.meta.env.VITE_BACKEND_URL || 'http://localhost:5004'}/api/auth/login`, { email });

      if (res.data.success) {
        setMessage('Login successful');
        localStorage.setItem('user', JSON.stringify(res.data.user));
        window.location.href = '/'; // Redirect to certificate page
      } else {
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-indigo-200 via-white to-indigo-300 p-6">
      ...
      <form
        ...
        className="w-full max-w-md bg-white rounded-3xl shadow-xl p-10
                   sm:p-12 border border-indigo-400"
      >
        ...
        <input
          ...
          className="w-full p-4 mb-6 rounded-xl border border-indigo-300
        ...
        <button
          type="submit"
          className="w-full bg-indigo-600 hover:bg-indigo-900 text-white
                     font-semibold py-3 rounded-xl shadow-lg transition duration-300"
        >
          Login
        </button>
